...
function Navbar({ onMenuClick }) {
  return (
    <header className="sticky top-0 z-40 h-14 border-b border-[var(--color-accent)]/30 bg-white/85 backdrop-blur supports-[backdrop-filter]:bg-white/60">
      <div className="mx-auto flex h-14 max-w-screen-2xl items-center gap-3 px-4">
        <button onClick={onMenuClick} className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-[var(--color-accent)]/40 text-[var(--color-primary)] hover:bg-[var(--color-accent)]/15 md:hidden" aria-label="Open menu">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-[var(--color-accent)]/40 bg-white text-[var(--color-primary)] shadow-sm hover:bg-[var(--color-accent)]/15" aria-label="Notifications">
            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M6 8a6 6 0 0 1 12 0c0 7 3 9 3 9H3s3-2 3-9"/><path d="M10.3 21a1.94 1.94 0 0 0 3.4 0"/></svg>
          </button>
          <Link to="/settings" className="h-8 w-8 rounded-full border border-[var(--color-accent)]/40 bg-[var(--color-accent)]/30" aria-label="Profile" />
        </div>
      </div>
    </header>
  )
